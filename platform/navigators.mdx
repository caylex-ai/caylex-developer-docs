...
When enabled, the Navigator exposes the `execute_tools_in_code` MCP tool. This lets your agent generate and run sandboxed Python to orchestrate tool calls, process large responses, and return only the relevant result to the model.

Toggle **Code Mode** under **Caylex Capabilities** on the Navigator Instance page.

<Card title="Learn more about Code Mode" icon="code" href="/navigator/code-mode">
  Understand how Code Mode works and when to enable it.
</Card>
