...
1. Your agent connects to the Caylex Navigator at `https://navigator.caylex.ai/mcp` using Streamable HTTP transport
...
The Caylex Navigator exposes MCP tools to your agent based on your [Navigator Instance configuration](/platform/navigators):
...
### `execute_tools_in_code`

Runs agent-generated Python in a restricted sandbox. The program can orchestrate tool calls and process their results before returning a compact output to the model.

This tool is available when **Code Mode** is enabled on the Navigator Instance.

<Card title="Code Mode" icon="code" href="/navigator/code-mode">
  Learn how Code Mode handles multi-tool workflows and large responses.
</Card>
