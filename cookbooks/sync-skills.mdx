...
Store your token as a CI secret and run the script on merges to `main`. A GitHub Actions step looks like:

```yaml .github/workflows/sync-skills.yml
name: Sync Caylex skills
on:
  push:
    branches: [main]
    paths: ["skills/**"]
jobs:
  sync:
    ...
    steps:
      ...
      - run: python sync_skills.py
        env:
          CAYLEX_PLATFORM_TOKEN: ${{ secrets.CAYLEX_PLATFORM_TOKEN }}
          CAYLEX_PROJECT: "Production Project"
```
