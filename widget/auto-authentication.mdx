...
Auto-authentication is an optional extension of the same `POST /widget/mint-token` call you already make to embed a widget. You do not call a separate endpoint.
...
sequenceDiagram
    ...
    App->>Caylex: POST /widget/mint-token (with auto_authenticate)
...
<Steps>
  ...
  <Step title="Include an auto_authenticate payload when minting the token">
    Add the `auto_authenticate` object to your existing mint-token request, naming the target server and supplying the raw header value(s).
  </Step>
...
Add an optional `auto_authenticate` object to your `POST /widget/mint-token` body:
