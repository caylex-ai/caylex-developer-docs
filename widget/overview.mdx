...
Your backend can send a normal user turn to an existing widget session. The
message and agent response are persisted and appear in the widget when external
session synchronization is enabled.

See [Session Management](/widget/session-management) to select an initial
session, capture new session IDs, react to session changes, and enable
programmatic turns.
...
Use `onSessionChanged` to detect when the user enters or leaves a background
task view. See [Session Management](/widget/session-management) for the callback
contract.
...
| `initialSessionId` | — | Session to restore during initialization. Pass `null` to create a new session; omit to preserve browser restoration. |
...
| `onSessionChanged` | — | Invoked with `{ sessionId, sessionType }` whenever the active session or session view changes |
...
The Caylex platform includes an **Embedded AI** page where you can:
...
Use the Embedded AI page to experiment with the widget before integrating it into your application.
