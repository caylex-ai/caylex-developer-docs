...
| Agent can't navigate pages | `navigablePages` is empty, the site map fetch failed, or `onNavigate` is missing | Ensure your backend site map endpoint returns pages and both props are set on `CaylexChatWidget` |
