...
sequenceDiagram
    ...
    App->>Caylex: POST /widget/agent-session-init
...
```html
<script src="https://assets.caylex.ai/widgets/chat-widget@0.1.17.js"></script>
...
Use this pattern when you want a chat launcher that opens a fixed popup. Keep the launcher markup in your root layout so the widget is available across pages.

```html floating-chat-widget.html
<button
  id="caylex-chat-launcher"
...
<div
  id="caylex-chat-popup"
  style="display: none; position: fixed; right: 24px; bottom: 24px; width: 420px; height: 600px; z-index: 1000; border-radius: 16px; overflow: hidden; box-shadow: 0 12px 40px rgba(0, 0, 0, 0.15);"
>
  <button
    id="caylex-chat-close"
    style="position: absolute; top: 12px; right: 12px; z-index: 1;"
  >
    Close
  </button>
  <div id="caylex-chat" style="width: 100%; height: 100%;"></div>
</div>

<script src="https://assets.caylex.ai/widgets/chat-widget.js"></script>
<script>
  let caylexChatInstance = null;
  ...
  async function openCaylexChat() {
    document.getElementById('caylex-chat-launcher').style.display = 'none';
    document.getElementById('caylex-chat-popup').style.display = 'block';

    if (caylexChatInstance) return;

    const token = await fetchWidgetToken();
    caylexChatInstance = CaylexChat.render(document.getElementById('caylex-chat'), {
      apiBaseUrl: 'https://api.caylex.ai/api/v1/assistants/',
      widgetToken: token,
      refreshToken: fetchWidgetToken,
      height: '100%',
  ...
  function closeCaylexChat() {
    document.getElementById('caylex-chat-popup').style.display = 'none';
    document.getElementById('caylex-chat-launcher').style.display = 'block';
  }

  document.getElementById('caylex-chat-launcher').addEventListener('click', () => {
    openCaylexChat().catch(console.error);
  });
  document.getElementById('caylex-chat-close').addEventListener('click', closeCaylexChat);
</script>
```
...
Your backend can send a normal user turn to an existing widget session. This is
useful when the embedded agent needs to follow application events or state
changes that happen outside the chat composer. The agent processes the message
normally, and the persisted user and assistant messages appear in the open
widget thread when external session synchronization is enabled:
...
External synchronization is disabled by default so widgets that do not use
programmatic turns make no additional polling requests. When enabled, polling
runs only while the tab is visible and the interactive session is idle.

Capture each new session ID with `onSessionCreated`, then call the
platform-authenticated session-message API from your backend. See
[Programmatic Chat Sessions](/cookbooks/programmatic-chat-sessions) for the
request contract, session lookup options, and complete examples.

## Background Task Approval Inbox

The chat widget can act as the human approval surface for [background agent tasks](/background-tasks/agent-tasks). No additional widget prop is required.

When your backend submits a task with `approval_mode: "human"`:
...
type NavigablePage = {
  ...
  description?: string;
  urlTemplate: string;
  templateParams?: Record<string, { description: string; required?: boolean }>;
...
The legacy `params` field remains supported as a deprecated alias for
...
queryParams: {
  'sales-item': {
    description: 'Filter by sales item',
  },
  'memo-out': {
    description: 'Memo filter',
    defaultValue: '', // Produces memo-out= when omitted by the agent
  },
}
```

Query parameter names and values are URL-encoded automatically. Parameters not
declared in `queryParams` are rejected for pages using this contract.
