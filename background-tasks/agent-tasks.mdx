...
<Note>
Both approval modes support programmatic decisions from your backend.
`approval_mode: "human"` is limited to tool approval decisions.
`approval_mode: "deferred_human"` can also request written feedback or a
decision on an important draft, schedule, document, or other work product.
</Note>
...
sequenceDiagram
    ...
    alt approval_mode: "human"
        ...
        Backend->>Caylex: poll pending_approval
        Backend->>Caylex: approve or reject batch
    ...
    opt continue completed work
        Backend->>Caylex: POST /agent-task/{task_id}/continue
        Caylex->>Worker: append feedback and resume same session
    end
```
...
After a task reaches `COMPLETED` or `INCOMPLETE`, your backend can provide a
new user message and continue the same task and chat session:

```http
POST https://api.caylex.ai/api/v1/agent-task/{task_id}/continue
Authorization: Bearer <platform_access_token>
...
```json
{
  "caylex_api_key": "ck_your_navigator_api_key",
  "user_email": "jane@example.com",
  "message": "Expand the report with a comparison of the previous quarter."
}
```
...
Continuation is accepted only for `COMPLETED` and `INCOMPLETE` tasks. It
appends the feedback as a normal user turn, requeues the task, and preserves
the existing transcript. Continue polling the original `task_id`; approvals
work exactly as they did during the first run.

You must provide the platform token, navigator API key, and original
`user_email` again. Caylex deletes the task's temporary navigator credential
when a run reaches a terminal state, then stores the newly supplied key only
...
2. `GET /agent-task/{task_id}` returns the pending batch in `pending_approval`. A chat widget initialized for the same navigator and `user_email` also shows it under **Background Tasks**.
3. Your backend can submit decisions through the API, or the end user can use the widget approval card.
4. After decisions are submitted, Caylex requeues the task, executes only approved actions, records rejected actions as tool results, and continues the workflow.
...
#### Immediate approval through the API

When the task is waiting, its status response includes the batch and individual
tool approval IDs:
...
```json
{
  ...
  "pending_approval": {
    "batch_id": "7fb90b23-93c2-45a9-98c4-1572fed2efc9",
    "expires_at": "2026-09-17T06:32:29Z",
    "items": [
      {
        "approval_id": "71bf58ab-80bd-488b-b85e-5b1e9dc91cc6",
        "tool_name": "notion-create-pages",
        "server_name": "Notion",
        "intent": "Create the researched report",
        "tool_params": {}
      }
    ]
  }
...
Submit the decision using the batch ID from the status response:

```http
POST https://api.caylex.ai/api/v1/agent-task/{task_id}/approvals/{batch_id}/decide
Authorization: Bearer <platform_access_token>
Content-Type: application/json
```
...
Each action is either `approved` or `rejected`. Items omitted from an immediate
approval submission are rejected. A successful submission returns
`{"status":"ok"}` and requeues the task.
