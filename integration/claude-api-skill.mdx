---
title: "Claude API Agent Skill"
description: "A ready-made SKILL.md that instructs your coding agent to wire a FastMCP client for the Caylex Navigator into your Claude Messages API harness."
icon: "wand-magic-sparkles"
---

This page contains a complete [Agent Skill](https://code.claude.com/docs/en/skills) for coding agents such as Claude Code or Cursor. Feed it to your coding agent and it will add a customer-managed FastMCP client to your existing Claude Messages API application, following the recommended approach from the [Claude API integration guide](/integration/claude-api).
...
<Steps>
  ...
  <Step title="Invoke it">
    ...
    ```text
    Use the integrate-caylex-fastmcp-claude-api skill to connect our
    Claude chat agent to the Caylex Navigator.
    ```
  </Step>
  <Step title="Review the changes">
    The skill includes a verification checklist the agent should satisfy. Confirm your `CAYLEX_API_KEY` is loaded from server-side secrets and that the end-user email matches your [Auth Link](/auth/auth-links) setup.
  </Step>
</Steps>
